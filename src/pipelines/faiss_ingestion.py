...
class FaissIngestionPipeline:
    def __init__(self, embedding_client: EmbeddingModel, faiss_index: FaissIndex, auto_fix_id=True):
    ...
    async def ingest_dataset(self, dataset: List[Dict], batch_size: int = 8):
        ...
        for i in tqdm(range(0, len(dataset), batch_size), desc="Embedding + Ingest"):
            ...
            self.faiss_index.add_batch(batch_vectors, batch_meta)

        self.faiss_index.save()
        
    def _fix_ids_if_needed(self, dataset):
        if len(self.faiss_index.metadata) == 0:
            return dataset
        
        existing_ids = [meta["id"] for meta in self.faiss_index.metadata]
